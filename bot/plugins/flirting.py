...
from models.flirting import FlirtingStats, FlirtingAchievement
from services.acn_service import acn_only
from services.flirting_service import get_flirting_service, process_flirting_command
...
async def flirt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    result = await process_flirting_command(
    ...
    if result["success"]:
        ...
        if "response" in result:
            ...
            await msg.reply_text("📖 *adjusts glasses* I'm not quite sure how to respond to that...")
    ...
    async with async_session_factory() as session:
        ...
        result = await session.execute(
            select(FlirtingStats).where(
                FlirtingStats.user_id == user.id,
                FlirtingStats.group_id == chat.id
            )
        )
        ...
        if stats.last_flirt_time > 0:
            last_flirt = time.strftime("%Y-%m-%d %H:%M", time.localtime(stats.last_flirt_time))
...
async def flirt_categories(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response += "💡 **Tip:** Different categories work better with different approaches!\n"
...
@acn_only
async def flirt_achievements(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with async_session_factory() as session:
        ...
        for achievement in achievements:
            unlocked_time = time.strftime("%Y-%m-%d", time.localtime(achievement.unlocked_at))
...
async def flirt_random(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response = f"🌸 **Random Flirt Inspiration:**\n\n"
...
async def _check_flirting_achievements(update: Update, context: ContextTypes.DEFAULT_TYPE, result: dict) -> None:
