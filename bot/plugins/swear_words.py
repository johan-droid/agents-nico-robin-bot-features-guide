    ...
    try:
        async with async_session_factory() as session:
            async with session.begin():
                ...
                swear_word = await SwearWordService.add_swear_word(
                    session=session,
                    group_id=chat.id,
                    word=word,
                    severity=severity,
        ...
        await AuditService.log_action(
            ...
            extra={"word": word, "severity": severity, "punishment": punishment, "duration": duration},
        )
...
async def remove_swear_word(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        async with async_session_factory() as session:
            async with session.begin():
                deleted_count = await SwearWordService.remove_swear_word(session, chat.id, word)
...
async def list_swear_words(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        for swear_word in swear_words:
            duration_text = f" ({human_duration(swear_word.duration)})" if swear_word.duration > 0 else ""
...
async def swear_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(args) == 0:
        try:
            ...
            duration_text = f" ({human_duration(group.default_swear_duration)})" if group.default_swear_duration > 0 else ""
            await msg.reply_text(
    ...
    try:
        ...
        await AuditService.log_action(
            ...
            actor_id=update.effective_user.id if update.effective_user else None,
            target_id=None,
            reason=f"Updated swear word settings",
            extra={"severity": severity, "punishment": punishment, "duration": duration},
        )
    ...
    try:
        if punishment.action == "mute":
            ...
            if punishment.duration > 0:
                asyncio.create_task(_schedule_unmute(context, chat.id, user.id, punishment.duration))
            
        elif punishment.action == "temp_ban":
            await context.bot.ban_chat_member(chat.id, user.id)
            # Schedule unban if duration > 0
            if punishment.duration > 0:
                asyncio.create_task(_schedule_unban(context, chat.id, user.id, punishment.duration))
...
async def _schedule_unmute(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int, duration: int) -> None:
...
async def _schedule_unban(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int, duration: int) -> None:
...
async def handle_swear_words(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        async with async_session_factory() as session:
            async with session.begin():
                ...
                # Match swear words
                matches = await SwearWordService.match_swear_words(session, chat.id, msg.text)
                if not matches:
                    return
