...
class ACNWhitelist(Base, TimestampMixin):
    ...
    whitelist_id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    whitelist_type: Mapped[str] = mapped_column(String(20), nullable=False)  # 'group' or 'user'
    entity_id: Mapped[int] = mapped_column(BigInteger, nullable=False, index=True)  # group_id or user_id
    entity_name: Mapped[str] = mapped_column(String(255), nullable=False)  # group name or username
    role: Mapped[str] = mapped_column(String(50), nullable=False)  # 'captain', 'commander', 'member', 'ally'
    ...
    # Relationships
    group: Mapped[Group | None] = relationship(foreign_keys=[entity_id], primaryjoin="and_(ACNWhitelist.entity_id==Group.group_id, ACNWhitelist.whitelist_type=='group')")
    user: Mapped[User | None] = relationship(foreign_keys=[entity_id], primaryjoin="and_(ACNWhitelist.entity_id==User.user_id, ACNWhitelist.whitelist_type=='user')")
...
class LoyaltyPoints(Base, TimestampMixin):
    ...
    last_activity: Mapped[int] = mapped_column(BigInteger, default=0, nullable=False)  # Unix timestamp
...
class LoyaltyReward(Base, TimestampMixin):
    ...
    reward_type: Mapped[str] = mapped_column(String(50), nullable=False)  # 'title', 'role', 'permission', 'custom'
    reward_data: Mapped[str | None] = mapped_column(Text, nullable=True)  # JSON string
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    required_role: Mapped[str] = mapped_column(String(50), default="member", nullable=False)
...
class LoyaltyRedemption(Base, TimestampMixin):
    ...
    processed_at: Mapped[int] = mapped_column(BigInteger, nullable=True)  # Unix timestamp
...
class ACNActivity(Base, TimestampMixin):
    ...
    activity_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)  # 'moderation', 'engagement', 'loyalty'
