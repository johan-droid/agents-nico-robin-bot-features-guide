...
def _verify_hmac_token(user_id: str, timestamp: str, token: str) -> bool:
    ...
    try:
        ts = float(timestamp)
        if abs(time.time() - ts) > 300: return False  # 5 min window
...
class WebSocketManager:
    ...
    async def _on_connect(self, sid: str, environ: Dict[str, Any]):
        ...
        asyncio.get_event_loop().call_later(10.0, lambda: asyncio.ensure_future(self._auth_timeout(sid)))
    ...
    async def _auth_timeout(self, sid: str):
        if sid not in self._authenticated and sid in self._connect_times:
            ...
            try: await self.sio.disconnect(sid)
    ...
    async def _on_authenticate(self, sid: str, data: Dict[str, Any]):
        ...
        if not user_id or not token or not timestamp:
            await self.sio.emit('auth_error', {'error': 'Missing credentials'}, room=sid)
            return
        if not _verify_hmac_token(str(user_id), str(timestamp), str(token)):
            ...
            try:
                ...
                await SecurityLogger.log_event("ws_auth_failed", user_id=int(user_id) if str(user_id).lstrip('-').isdigit() else None, severity="MEDIUM")
            except Exception: pass
    ...
    async def _on_join_room(self, sid: str, data: Dict[str, Any]):
        ...
        if room_id == "admins":
            ...
            if uid:
                ...
                if uid_int not in settings.sudo_users and uid_int != settings.captain_id and uid_int not in settings.commander_ids:
                    await self.sio.emit('error', {'error': 'Admin room restricted'}, room=sid)
                    return
        await self.sio.enter_room(sid, room_id)
        if room_id not in self.active_connections: self.active_connections[room_id] = set()
        self.active_connections[room_id].add(sid)
        if sid not in self.session_rooms: self.session_rooms[sid] = set()
    ...
    async def _on_leave_room(self, sid: str, data: Dict[str, Any]):
        ...
        if sid in self.session_rooms: self.session_rooms[sid].discard(room_id)
...
async def emit_realtime_event(event_type: str, data: Dict[str, Any], target: str = "global", target_id: str = None):
    event = {'event': 'realtime_update', 'type': event_type, 'data': data,
             'timestamp': asyncio.get_event_loop().time(), 'target': target, 'target_id': target_id}
    if target == "group" and target_id: await websocket_manager.broadcast_to_group(target_id, event)
    elif target == "user" and target_id: await websocket_manager.send_to_user(target_id, event)
    elif target == "admins": await websocket_manager.broadcast_to_admins(event)
    else: await websocket_manager.broadcast_global(event)

USER_EVENTS = {'user_joined','user_left','user_banned','user_unbanned','user_warned','user_muted','user_unmuted'}
MODERATION_EVENTS = {'message_deleted','filter_triggered','swear_word_violation','ai_moderation','flood_detected'}
GROUP_EVENTS = {'settings_changed','filter_added','filter_removed','welcome_updated','rules_updated'}
SYSTEM_EVENTS = {'bot_status','performance_alert','error_occurred','maintenance_mode','channel_broadcast','broadcast_error'}

async def emit_user_event(event_type: str, user_id: str, group_id: str, data: Dict[str, Any]):
    if event_type not in USER_EVENTS: return
