...
class PointService:
    ...
    EARNING_ACTIVITIES = {
        ...
        "flirt_success": {"points": 5, "description": "Successful flirting", "cooldown": 300},
        "bond_interaction": {"points": 3, "description": "Bot friendship interaction", "cooldown": 600},
        "daily_streak": {"points": 10, "description": "Daily activity streak", "cooldown": 86400},
        "weekly_streak": {"points": 50, "description": "Weekly activity streak", "cooldown": 604800},
        "achievement": {"points": 20, "description": "Earning achievements", "cooldown": 0},
        "bonus": {"points": 15, "description": "Special bonuses", "cooldown": 0},
        "helpful": {"points": 8, "description": "Helpful contributions", "cooldown": 3600},
            ...
            async with session.begin():
                ...
                if not user_points:
                    ...
                    await self._award_points(
                        session, user_id, group_id, 50, "bonus", 
    ...
    async def _update_level_and_experience(self, user_points: UserPoints, points_earned: int):
    ...
    async def get_user_points(self, user_id: int, group_id: int) -> Optional[Dict]:
    ...
    async def get_user_apploids(self, user_id: int, group_id: int) -> List[Dict]:
        ...
        async with async_session_factory() as session:
            result = await session.execute(
                select(UserApploid, Apploid).join(Apploid).where(
                    UserApploid.user_id == user_id,
                    UserApploid.group_id == group_id
                )
            )
            apploids = result.all()
    ...
    async def get_available_apploids(self, user_id: int, group_id: int) -> List[Dict]:
        ...
        owned_ids = {app["apploid_id"] for app in owned_apploids}
        
        # Get all apploids user can access
        available = []
        for apploid_data in self.DEFAULT_APPLOIDS:
            if (apploid_data["required_level"] <= user_level and 
                apploid_data["required_points"] <= current_points and
                apploid_data["name"] not in [app["name"] for app in owned_apploids]):
    ...
    async def purchase_apploid(
        ...
        async with async_session_factory() as session:
            async with session.begin():
                ...
                # Check requirements
                if (apploid_data["required_level"] > user_points_info["level"] or
                    apploid_data["required_points"] > user_points_info["current_points"]):
                ...
                success, message = await self.spend_points(
                    session, user_id, group_id, apploid_data["required_points"],
                    "apploid_purchase", f"Purchased {apploid_name}"
                )
    ...
    async def get_leaderboard(self, group_id: int, limit: int = 10) -> List[Dict]:
        ...
        async with async_session_factory() as session:
            result = await session.execute(
                select(UserPoints).where(
                    UserPoints.group_id == group_id
                ).order_by(UserPoints.current_points.desc()).limit(limit)
            )
    ...
    async def process_activity(
        ...
        async with async_session_factory() as session:
            success = await self.award_points(
                session, user_id, group_id, points_earned,
                activity_type, description or activity["description"]
            )
