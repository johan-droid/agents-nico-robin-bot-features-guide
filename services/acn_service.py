...
class ACNService:
    ...
    @staticmethod
    async def is_admin_or_owner(user_id: int, chat: Chat, context: ContextTypes.DEFAULT_TYPE) -> bool:
        """Check if user is admin, owner, captain, or commander"""
        # Check captain/commander status first (highest priority)
        if await ACNService.is_captain(user_id) or await ACNService.is_commander(user_id):
            return True
    ...
    @staticmethod
    async def get_loyalty_points(session: AsyncSession, user_id: int, group_id: int) -> Optional[LoyaltyPoints]:
    ...
    async def get_group_members(session: AsyncSession, group_id: int) -> list[dict]:
        ...
        result = await session.execute(
            select(ACNWhitelist, LoyaltyPoints).join(
                LoyaltyPoints,
...
def captain_commander_only(func):
    ...
    async def wrapper(update, context, *args, **kwargs):
        ...
        if not (await ACNService.is_captain(user.id) or await ACNService.is_commander(user.id)):
            if update.message:
                await update.message.reply_text("🚫 Only Monkey D. Sparrow and commanders can use this command.")
            return
...
def admin_captain_commander_only(func):
    ...
    async def wrapper(update, context, *args, **kwargs):
        ...
        if await ACNService.is_captain(user.id) or await ACNService.is_commander(user.id):
            return await func(update, context, *args, **kwargs)
        ...
        try:
            ...
            if not await is_telegram_admin(context, chat.id, user.id):
                if update.message:
                    await update.message.reply_text("🚫 Only admins, Monkey D. Sparrow, and commanders can use this command.")
                return
        except Exception:
            if update.message:
