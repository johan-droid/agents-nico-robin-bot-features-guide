...
class EventService:
    ...
    async def create_event(
        ...
        if broadcast_immediately:
            asyncio.create_task(
                EventService._broadcast_event(event.id)
            )
        ...
        try:
            async with async_session_factory() as session:
                async with session.begin():
                    ...
                    logger.info(f"Broadcasted event {event_id} ({event.event_type}) to {recipients_count} recipients")
    ...
    @staticmethod
    async def unregister_websocket_connection(session: AsyncSession, connection_id: str):
    ...
    @staticmethod
    async def cleanup_inactive_connections(session: AsyncSession, timeout_minutes: int = 5):
        ...
        logger.info(f"Cleaned up inactive connections older than {timeout_minutes} minutes")
    ...
    async def unsubscribe_from_events(
        ...
        await session.execute(
            ...
            .where(
                ...
                EventSubscription.group_id == group_id if group_id is not None else EventSubscription.group_id.is_(None)
            )
        )
    ...
    async def get_connection_stats(session: AsyncSession) -> Dict[str, Any]:
        ...
        total_result = await session.execute(
            select(WebSocketConnection).where(WebSocketConnection.is_active == True)
        )
        ...
        group_result = await session.execute(
            ...
            .where(WebSocketConnection.is_active == True)
        ...
        recent_result = await session.execute(
            ...
            .where(RealtimeEvent.created_at >= datetime.now(UTC).timestamp() - 3600)  # Last hour
        ...
        return {
            ...
            'group_connections': {
                str(gid): len(users) for gid, users in group_connections.items() if gid is not None
            },
