...
class NicoRobinFlirtingService:
    ...
    async def process_flirting_attempt(
        ...
        if success:
            ...
            response = random.choice(matched_event["responses"][:2])  # Use first 2 as fallback
    ...
    def _find_matching_event(self, message_text: str) -> Optional[Dict]:
    ...
    async def _calculate_flirting_success(
        ...
        try:
            async with async_session_factory() as session:
                ...
                base_success = self.skill_categories[self._get_event_category(event["id"])]["success_rate"]
                ...
                final_success_rate = base_success + rank_bonus + points_bonus + random_factor
                final_success_rate = max(0.1, min(0.95, final_success_rate))  # Clamp between 10% and 95%
    ...
    def get_random_flirt_line(self, category: Optional[str] = None) -> str:
