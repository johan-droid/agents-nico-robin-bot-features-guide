    ...
    try:
        async with engine.begin() as conn:
            ...
            expected_tables = {
                ...
                # Feature management
                'feature_toggles', 'feature_permissions', 'feature_usage', 'feature_logs',
                
                # Flirting system
                'flirting_attempts', 'flirting_stats', 'flirting_achievements',
                'flirting_relationships', 'flirting_gifts', 'flirting_preferences',
                
                # Bot friendship
                'bot_friendships', 'bot_interactions', 'bot_memories', 'bot_gifts',
                'bot_conversations', 'bot_emotions',
                
                # Point system
                'user_points', 'point_transactions', 'apploids', 'user_apploids',
                'point_rewards', 'point_redemptions', 'point_streaks', 'point_leaderboards',
                
                # Additional features
                'managed_channels', 'member_profiles',
                
                # Existing tables
                'action_logs', 'join_logs', 'message_logs', 'filters', 'notes',
                'warns', 'swear_words', 'swear_violations', 'loyalty_points',
                'loyalty_redemptions', 'loyalty_rewards', 'acn_activities',
                'acn_whitelists', 'federations', 'federation_admins',
                'federation_bans', 'federation_groups', 'event_audit_logs',
                'event_subscriptions', 'realtime_events', 'websocket_connections'
            }
            ...
            key_validations = [
                ('feature_toggles', ['toggle_id', 'group_id', 'feature_name', 'is_enabled']),
                ('flirting_attempts', ['attempt_id', 'user_id', 'group_id', 'success']),
                ('bot_friendships', ['friendship_id', 'companion_bot_id', 'friendship_level']),
                ('user_points', ['points_id', 'user_id', 'group_id', 'current_points']),
                ('apploids', ['apploid_id', 'apploid_name', 'rarity']),
                ('member_profiles', ['profile_id', 'user_id', 'group_id', 'total_messages'])
            ]
            ...
            fk_validations = [
                ...
                ('user_apploids', ['users', 'groups', 'apploids'], ['user_id', 'group_id', 'apploid_id']),
                ('member_profiles', ['users', 'groups'], ['user_id', 'group_id'])
            ]
            
            for table_name, expected_refs, fk_columns in fk_validations:
                if table_name in tables:
            ...
            try:
                ...
                # Clean up test data
                await conn.execute(text("DELETE FROM apploids WHERE apploid_name = 'Test Apploid'"))
                await conn.execute(text("DELETE FROM feature_permissions WHERE feature_name = 'test_feature'"))
    ...
    try:
        async with engine.begin() as conn:
            ...
            categories = {
                ...
                'Feature Management': ['feature_toggles', 'feature_permissions', 'feature_usage', 'feature_logs'],
                'Flirting': [t for t in tables if t.startswith('flirting')],
                'Bot Friendship': [t for t in tables if t.startswith('bot')],
                'Point System': [t for t in tables if 'point' in t or t in ['apploids', 'user_apploids']],
                'Other': []
            }
