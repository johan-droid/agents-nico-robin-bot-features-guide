...
class BotWebSocketClient:
    ...
    async def connect(self, bot_user_id: int):
        ...
        try:
            ...
            await self.sio.connect(
                ...
                auth={'bot_id': bot_user_id, 'token': 'bot_token'}  # TODO: Use proper auth
            )
    ...
    async def _handle_moderation_event(self, event_type: str, event_data: Dict[str, Any]):
        ...
        if target_user_id:
            logger.info(f"Moderation action {action} on user {target_user_id} in group {group_id}")
...
bot_websocket_client: Optional[BotWebSocketClient] = None
...
def get_websocket_client() -> Optional[BotWebSocketClient]:
